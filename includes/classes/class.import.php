		...
		while( ($item = fgetcsv($csv, 0, ",", '"') ) !== FALSE ) {
			...
			if(!empty($spknumber)) { 
				...
				$lastid = $database->insert("spk", [
					...
					"id_itfs" => $id_itfs,
					"id_sn_edc" => $idedc,
					"reported_time" => dateTimeDisplay($item[12]),
					"received_time" => dateTimeDisplay($item[13]),
					"wo_activity" => $item[14],
					"reason_code" => $item[15],
					"spk_status" => $item[16],
					"wo_remarks" => $item[17],
					"remarks_spk" => $item[18],
					"received_date_sticker" => dateTimeDisplay($item[24]),
					"target_spk" => dateTimeDisplay($item[25]),
					"target_bank" => dateTimeDisplay($item[26]),
					"city" => $item[27],
