...
switch($_POST['action']) {
	...
	case "addCustomer":
		isAuthorized("addClient"); $status = Customer::add($_POST);
	break;

	case "editCustomer":
		isAuthorized("editClient"); $status = Customer::edit($_POST);
	break;

	case "deleteCustomer":
		isAuthorized("deleteClient"); $status = Customer::delete($_POST['id']);
    break;
...
// Provider
	...
	case "importEditSPK":
		$status = Import::editspk($_POST, $_FILES);
	break;
